...
import datasets
from utils.trainer_vanilla_style_contrast import Trainer
...
def get_train_loader(args, dataset, height, width, batch_size, workers, iters):
...
def online_clustering(source, model, epoch, num_clustering):
    ...
    pseudo_domain_label = domain_split(source, model, device=device,
                                       ...
                                       nmb_cluster=num_clustering, method='Spectral',
...
def main_worker(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1):
            ...
            if is_best:
                ...
                torch.save(saved_model, osp.join(args.logs_dir, args.save_best))
            torch.save(saved_model, osp.join(args.logs_dir, args.save_latest))
    ...
    checkpoint = torch.load(osp.join(args.logs_dir, args.save_best))
    model.load_state_dict(checkpoint['state_dict'])
...
if __name__ == '__main__':
    ...
    # model
    parser.add_argument('-a', '--arch', type=str, default='memMeta',
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                        default=osp.join('dataset/SHA-SHB'))
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join('', 'logs'))
    parser.add_argument('--resume', type=str, default='')
    parser.add_argument('--evaluate', action='store_true',
                        help="evaluation only")
    parser.add_argument('--save-best', type=str, default="bestmodel_SHA-SHB.pth.tar")
    parser.add_argument('--save-latest', type=str, default="latestmodel_SHA-SHB.pth.tar")
    main()
